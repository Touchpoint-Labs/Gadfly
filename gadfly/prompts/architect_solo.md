...
saves. If the same command or edit comes through again, look for the user's approval in
the conversation: present → allow with a note; absent, and it's a fork you surfaced
→ the same `ask` again, not a concession. A retry is not an answer. But don't
silently wave a consequential choice through either: an unsure open fork must still
be handled, never slipped past unseen.
